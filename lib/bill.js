...
exports.showTypeDataByDate = function(db, res, paras) {
    ...
    if (paras.startDate.length == 6 && paras.endDate.length == 6) {
...
exports.showMonthBillsByDate = function(db, res, paras) {
    ...
    var query = "SELECT LEFT(b.time,6) as t,cast(sum(b.money) as decimal(10,2)) as sumMoney from bill_info  b left join income_expenditure_type t on b.type_id=t.type_id WHERE b.user_id in (" +
        ...
        user_sql + tab_sql +
        "GROUP BY substring(b.time,5,2) ORDER BY t desc"
    db.query(
        query, parasArray,
        function(err, rows) {
            if (err) badRequestCatch(res, err);
...
exports.showUserBalance = function(db, res, paras) {
    var query = "SELECT b.user_id, cast(b.balance as decimal(10,2))-cast(p.pay as decimal(10,2))+cast(i.income as decimal(10,2)) as bmoney from balance b left join (" +
        "select b.user_id,SUM(b.money) as pay from bill_info b left join income_expenditure_type i on b.type_id=i.type_id left join balance ba on b.user_id = ba.user_id WHERE  (i.tab='0' or i.tab='2') and b.time>ba.time GROUP BY b.user_id) p on b.user_id = p.user_id " +
        "left join (select b.user_id,SUM(b.money) as income from bill_info b left join income_expenditure_type i on b.type_id=i.type_id left join balance ba on b.user_id = ba.user_id WHERE (i.tab='1' or b.get_user_id='an') and b.time>ba.time GROUP BY b.user_id) i on b.user_id = i.user_id" +

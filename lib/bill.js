...
exports.showUserBalance = function(db, res, paras) {
    ...
    var query = "SELECT b.user_id, cast(b.balance as decimal(10,2))-cast(p.pay as decimal(10,2))+cast(i.income as decimal(10,2)) as bmoney from balance b left join (" +
        ...
        "left join (select b.user_id,SUM(b.money) as income from bill_info b left join income_expenditure_type i on b.type_id=i.type_id left join balance ba on b.user_id = ba.user_id WHERE (i.tab='1' or b.get_user_id=ba.user_id) and b.time>ba.time " + time_sql + " GROUP BY b.user_id) i on b.user_id = i.user_id" +
